...
func TestCreate_JSONAPIErrorSurfacing(t *testing.T) {
	mt := &mockTransport{responses: []mockResponse{
		{422, `{"errors":[{"status":"422","detail":"name has already been taken"}]}`},
	}}
	ctx, _ := buildCtx(t, mt, false)
	parent := projects.NewCommand()
	sub, _, err := parent.Find([]string{"create"})
	...
	sub.SetContext(ctx)
	if err := sub.ParseFlags([]string{"--name", "dup"}); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(err.Error(), "name has already been taken") {
		t.Errorf("expected error to contain 'name has already been taken', got: %v", err)
	}
}
